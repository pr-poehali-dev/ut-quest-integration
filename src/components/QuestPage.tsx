...
async function syncProgress(level: number, score: number, phone: string) {
  try {
    ...
    // Синхронизация в фоне — не блокирует квест
  }
}
...
export default function QuestPage({ user, onLoginClick }: Props) {
  ...
  const [status, setStatus] = useState<'idle' | 'correct' | 'wrong' | 'finish'>('idle');
  ...
  const handleCheck = () => {
    ...
    if (normalized === quest.answer.toLowerCase()) {
      const gained = hintUsed ? quest.points - 2 : quest.points;
      const newScore = score + Math.max(0, gained);
      setScore(newScore);
      setStatus('correct');
      setCompletedLevels((prev) => [...prev, quest.id]);
      triggerRipple();
      if (user) syncProgress(currentIdx + 1, newScore, user.phone);
    } else {
      setAttempts((a) => a + 1);
      setStatus('wrong');
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-6">
        ...
        >
          ...
          <h2 className="font-oswald font-bold text-2xl uppercase mb-2" style={{ color: 'white' }}>
            Путь закрыт
          </h2>
          <p className="text-sm mb-6" style={{ color: 'rgba(255,255,255,0.55)' }}>
            Войдите в аккаунт, чтобы начать квест и сохранить свой прогресс в рейтинге.
          </p>
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center px-6 py-16">
        <div className="text-center max-w-lg animate-scale-in">
          <div
            className="w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6 animate-glow-pulse"
            style={{ backgroundColor: 'rgba(255,209,102,0.15)', border: '2px solid var(--gold)' }}
          >
            <Icon name="Trophy" size={40} style={{ color: 'var(--gold)' }} />
          </div>
          <h2 className="font-oswald font-bold text-4xl uppercase mb-2" style={{ color: 'var(--gold)' }}>
            Путь пройден!
          </h2>
          ...
          <div
            className="inline-block px-6 py-3 rounded-xl my-4"
          ...
          <p className="text-sm mb-8" style={{ color: 'rgba(255,255,255,0.55)' }}>
            Пройдено загадок: {completedLevels.length} из {QUESTS.length}
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            <button className="btn-gold">
              <span className="flex items-center gap-2">
                <Icon name="Download" size={16} />
                Скачать путеводитель PDF
              </span>
            </button>
            <button
              className="flex items-center gap-2 px-5 py-3 rounded-lg text-sm font-oswald font-semibold uppercase tracking-widest"
              style={{ border: '1px solid rgba(64,195,176,0.3)', color: 'var(--teal)' }}
              onClick={() => {
                setCurrentIdx(0); setScore(0); setStatus('idle');
                setCompletedLevels([]); setAnswer(''); setHintShown(false); setHintUsed(false);
              }}
            >
          ...
          >
            🗝 Ключ к интерактивной карте: <strong>UI-{score}-{user.phone.slice(-4)}</strong>
          </div>
        </div>
      </div>
    );
    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          ...
          <span
            className="font-oswald font-bold text-sm px-3 py-1 rounded-full"
        ...
        <div className="flex gap-1 mt-2 flex-wrap">
          {QUESTS.map((q, i) => (
            <div
              key={q.id}
              className="h-1 rounded-full flex-1 min-w-[8px] transition-colors duration-300"
              style={{
      ...
      >
        ...
        {ripple && (
          ...
          >
            <div
              className="w-16 h-16 rounded-full animate-ripple"
              style={{ backgroundColor: 'rgba(64,195,176,0.4)' }}
            />
          </div>
        )}
        ...
        {quest.mediaUrl && (
          <div className="relative h-52 md:h-64 overflow-hidden">
            ...
            <div
              ...
              style={{ background: 'linear-gradient(to bottom, transparent 50%, var(--forest-mid) 100%)' }}
            />
            <div
              className="absolute top-3 left-3 px-2 py-1 rounded text-xs uppercase tracking-wider font-medium"
              style={{ backgroundColor: 'rgba(13,31,20,0.7)', color: 'var(--teal)', border: '1px solid rgba(64,195,176,0.3)' }}
            >
              <Icon name="Image" size={10} className="inline mr-1" />
              Найди ответ на фото
            </div>
          </div>
        )}
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-3 mb-4">
            <div
              className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-oswald font-bold"
          ...
          {!hintShown ? (
            ...
            >
              ...
              Показать подсказку (−2 очка)
            </button>
          ) : (
            <div
              className="flex items-start gap-2 px-4 py-3 rounded-lg mb-5 animate-fade-in-fast"
              ...
              <input
                ...
                style={{
                  ...
                  border: status === 'wrong'
                    ...
                    : '1px solid rgba(64,195,176,0.2)',
                  color: 'white',
                }}
              />
              <button
                className="btn-teal px-5 py-3 text-sm"
                onClick={handleCheck}
          ...
          {status === 'wrong' && (
            <p className="mt-2 text-xs flex items-center gap-1 animate-fade-in-fast" style={{ color: '#ef4444' }}>
              ...
              {attempts >= 2 && !hintShown && (
                <span style={{ color: 'rgba(255,255,255,0.45)' }}> — может, воспользоваться подсказкой?</span>
              )}
            </p>
          )}
        </div>
      ...
      >
        <div>
          ...
          <p className="text-xs" style={{ color: 'rgba(255,255,255,0.35)' }}>Поддержи проект донатом</p>
        </div>
        <a
          href="https://yoomoney.ru/to/410017253212598/0"
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all duration-200 hover:opacity-80"
